...
export const FetchLeads = ({ page, search, employeeIDs, rows, startDate, endDate, selectedSources }: { page: number; search: string; employeeIDs: string[]; rows: number; startDate: string; endDate: string; selectedSources: string[] }) => {
	return useQuery({ queryKey: ["leads", page, search, employeeIDs, rows, startDate, endDate, selectedSources], queryFn: () => getLeads({ page, search, employeeIDs, rows, startDate, endDate, selectedSources }), placeholderData: keepPreviousData });
};
...
export const FetchReminders = (id: string) => {
	return useQuery({ queryKey: ["reminders", id], queryFn: () => getReminders(id) });
};
