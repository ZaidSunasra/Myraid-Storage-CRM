...
import { FetchDealByDuration } from "@/api/deals/deal.queries";

const DealAnalytics = () => {
	const [period, setPeriod] = useState<"today" | "weekly" | "monthly" | "yearly" | "all">("today");

	const { data, isPending, isError } = FetchDealByDuration(period);
					...
					<div className="space-y-2">
						<div className="flex gap-4 items-center">
							<div className="flex items-center space-x-2">
								...
								<CardTitle className="text-lg">Deals Analytics</CardTitle>
							</div>
						...
						<CardDescription>Deal generation performance for</CardDescription>
					</div>
					<CardContent className="space-y-4">
						<div className="text-xl font-bold">
							Total Deals: <span className="text-primary">{data?.totalDeals}</span>
						</div>
						<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4">
							{Object.entries(data?.employeeDealCount || 0).map(([name, count]: [string, number], index: number) => {
