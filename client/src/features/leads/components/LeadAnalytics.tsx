        ...
        <CardHeader>
            <div className="flex flex-col md:flex-row gap-4">
                <div className="space-y-2">
                    ...
                    <CardDescription >
                        Lead generation performance for
                    </CardDescription>
                </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {Object.entries(data?.employeeLeadCount || 0).map(([name, count]: [string, number], index: number) => {
                            ...
                            return (
                                <div
                                    key={name}
                                    className={`rounded-xl p-4 ${color} flex flex-col justify-between shadow-sm`}
                                >
                                    <div className="flex justify-between items-center">
                                        <div className="text-lg font-semibold truncate">{name}</div>
                                        <div className={`rounded-full px-2 py-1 text-sm font-bold bg-white ${color}`}>
                                            {count}
                                        </div>
                                    </div>
